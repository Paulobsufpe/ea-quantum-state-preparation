...
if (len(sys.argv) > 1 and sys.argv[1] == "omp"):
    try:
        from qext_omp import GateType, CircuitIndividual, \
            QuantumEvolutionaryOptimizer
        from qext_omp import calculate_fidelity, calculate_fitness
        HAS_EXT = True
    ...
    try:
        from qext import GateType, CircuitIndividual, \
            QuantumEvolutionaryOptimizer
        from qext import calculate_fidelity, calculate_fitness
        HAS_EXT = True
    except ImportError as e:
        print(f"C++ extension not available: {e}")
        HAS_EXT = False
...
class VisualQuantumOptimizer:
    ...
    def __init__(self,
        ...
        self.gate_set = [GateType.ID, GateType.X,
        ...
        self.optimizer.set_fitness_function(
            lambda circuit: calculate_fitness(
                circuit, self.target_unitary, 10.0, 1.0, target_depth)
        )
    ...
    def run_optimization(self, verbose: bool = True) -> CircuitIndividual:
        ...
        self.best_circuit = self.optimizer.run_evolution(
            from_scratch=True, selection_method="roulette")
    ...
    def _compare_gate_distributions(self):
        ...
        print("  Gate       |  Target  |  Optimized |  Change")
        print("  -----------|----------|------------|---------")
...
def create_comparison_optimizer():
    """Create an optimizer for comparison between
       hybrid and standard approaches"""
